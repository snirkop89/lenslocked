...
type GalleryService struct {
	...
	// images. If not set, we use the path of ./images/
	ImagesDir string
}
...
func (service *GalleryService) Images(galleryID int) ([]Image, error) {
	globPattern := path.Join(service.galleryDir(galleryID), "*")
...
func (service *GalleryService) Image(galleryID int, filename string) (Image, error) {
	imagePath := path.Join(service.galleryDir(galleryID), filename)
...
func (service *GalleryService) CreateImage(galleryID int, filename string, content io.ReadSeeker) error {
	err := checkContentType(content, service.imageContentType())
	if err != nil {
