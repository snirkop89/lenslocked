...
func (g Galleries) Show(w http.ResponseWriter, r *http.Request) {
	...
	type Image struct {
		GalleryID       int
		Filename        string
		FilenameEscaped string // URL safe name
	}
	var data struct {
		ID     int
		Title  string
	...
	for _, fileHeader := range fileHeaders {
		...
		if err != nil {
			...
			if errors.As(err, &fileErr) {
				msg := fmt.Sprintf("%v has invalid content type or extension. Only png, gif and jpg files can be uploaded", fileHeader.Filename)
...
func userMustOwnGallery(w http.ResponseWriter, r *http.Request, gallery *models.Gallery) error {
	...
	if gallery.UserID != user.ID {
		...
		return fmt.Errorf("user does not have access to gallery")
	}
	return nil
}
